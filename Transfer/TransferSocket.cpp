	...
	default:
		{
			...
			while(m_nDataLen > 0)
			{
				...
				if(nUsed > 0)
				{
					...
					CnAssert(dwMsgID<=100000&&dwMsgID>0);
					...
					m_nDataLen -= nUsed;
					memmove_s(m_szNetBuf, ASYNC_SOCK_BUF_SIZE, m_szNetBuf + nUsed, m_nDataLen);
				}
				else if( nUsed == 0)
				{
...
bool TransferSocket::SendDataWithHead( const STNetMsgHeader* pstHeader, void* pData )
{
	...
	int32 nEncodeLen = g_BnFGameProto.Encode( pstHeader->dwMsgID, pData, pBodyData, nBodyLen);
