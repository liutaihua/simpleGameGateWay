...
{
  typedef
    typename boost::mpl::if_c<(::boost::is_const<T>::value), void const *, void *>::type
    type;
};
...
union max_align_for_funtion_pointer
{
  ...
  boost::mpl::if_c<( sizeof( generic_mfn_ptr ) < sizeof( mfn_ptr_s ) ), generic_mfn_ptr, mfn_ptr_s>::type dummy_mfp1;
  boost::mpl::if_c<( sizeof( generic_mfn_ptr ) < sizeof( mfn_ptr_m ) ), generic_mfn_ptr, mfn_ptr_m>::type dummy_mfp2;
  boost::mpl::if_c<( sizeof( generic_mfn_ptr ) < sizeof( mfn_ptr_v ) ), generic_mfn_ptr, mfn_ptr_v>::type dummy_mfp3;
  boost::mpl::if_c<( sizeof( generic_mfn_ptr ) < sizeof( mfn_ptr_u ) ), generic_mfn_ptr, mfn_ptr_u>::type dummy_mfp4;
};
...
static int compare_memory(delegate_holder const & dh_lhs, delegate_holder const & dh_rhs)
{
  if(dh_lhs.obj_ptr == dh_rhs.obj_ptr)
  {
    ...
    return 0 == memcmp_result
      // FIXED by Logan, to suppress sign-compare warning.
      ? static_cast<int>(ftable_base_lhs->size_of_fxn(dh_lhs.obj_ptr) - ftable_base_rhs->size_of_fxn(dh_rhs.obj_ptr))
      : memcmp_result;
  }
  else
  {
